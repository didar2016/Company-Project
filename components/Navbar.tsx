  ...
  return (
    <nav className="fixed w-[95%] sm:w-[90%] md:w-[85%] lg:w-[90%] z-50 bg-white/95 backdrop-blur-sm shadow-sm transition-all duration-300 top-4 sm:top-8 lg:top-12 left-1/2 -translate-x-1/2 rounded-full sm:rounded-[80px] md:rounded-[140px]">
      <div className="px-2 py-0 sm:px-3 sm:py-2 md:px-4 md:py-3 lg:px-8 lg:py-4">
        ...
        {/* Mobile Navigation */}
        {isOpen && (
          <div className="lg:hidden bg-gradient-to-b from-white to-gray-50 border-t border-gray-200/50 absolute left-0 right-0 top-full w-full overflow-y-auto max-h-[calc(100vh-6rem)] shadow-xl backdrop-blur-sm z-[55] touch-manipulation rounded-2xl">
            <div className="px-6 pt-6 pb-14 space-y-2 text-center flex flex-col items-center justify-start relative z-[56]">
                ...
                >
                  {link.subItems ? (
                    <div className="flex flex-col items-center">
                      ...
                      {activeDropdown === link.name && (
                        <div className="w-full py-2 mt-2 mb-2 animate-in slide-in-from-top-2 duration-200 relative z-[60]">
                          {link.subItems.map((subItem, subIndex) => (
                            <div key={subItem.name} className="relative px-4 py-1.5">
                              <Link
                                href={subItem.href}
                                className="block w-full py-3.5 px-4 text-base font-semibold text-gray-700 bg-white rounded-xl shadow-[0_2px_8px_rgba(0,0,0,0.06)] border border-gray-100 hover:shadow-[0_4px_12px_rgba(0,179,221,0.15)] hover:border-[#00B3DD] hover:text-[#00B3DD] hover:-translate-y-0.5 transition-all duration-300 text-center active:scale-[0.98]"
              ...
              <div className="pt-6">
                <Link
                  href="#book"
                  className="inline-flex items-center justify-center px-8 py-4 text-[#0095B8] border-[1.5px] border-[#0095B8] rounded-full text-base font-bold transition-all duration-300 uppercase tracking-wider transform hover:scale-105 min-w-[200px] cursor-pointer touch-manipulation active:bg-blue-100/50"
                  onClick={(e) => {
                    e.stopPropagation();
                    setIsOpen(false);
                    setActiveDropdown(null);
